...
class ImageModel(object):
    ...
    images_read_limit = 10  # Increase this to read more images
    ...
    def __init__(self, sc):
        ...
        print("Reading Model")
        model_path = os.path.join(self.model_dir, 'classify_image_graph_def.pb')
        ...
        node_lookup = self.NodeLookup(model_dir=self.model_dir).node_lookup
        # Broadcast node lookup table to use on Spark workers
        self.node_lookup_bc = sc.broadcast(node_lookup)

    def run_inference(self, sc, batched_data):
        print("Running Inference")
        #batched_data = self.read_file_index()
        #print("There are %d batches" % len(batched_data))

        urls = sc.parallelize(batched_data)
        labeled_images = urls.flatMap(self.apply_inference_on_batch)

        local_labeled_images = labeled_images.collect()
        return local_labeled_images
    ...
    def maybe_download_and_extract(self):
        ...
        if not os.path.exists(filepath):
            filepath2, _ = urllib.request.urlretrieve(self.MODEL_URL, filepath)
    ...
    class NodeLookup(object):
      ...
      def __init__(self,
                   ...
                   uid_lookup_path=None, model_dir=""):
    ...
    def read_file_index(self):
      ...
      if not os.path.exists("/tmp/imagenet.txt"):
        ...
        from six.moves import urllib
        content = urllib.request.urlopen(self.IMAGES_INDEX_URL)
        ...
        with open(tmpfile, 'wb') as f:
            f.write(data)
        self.run("tar -xOzf %s > /tmp/imagenet.txt" % tmpfile)
      ...
      with open("/tmp/imagenet.txt", 'r') as f:
          ...
          for line in f:
              ...
              if index > self.images_read_limit:
                  break
              index += 1

      input_data = [tuple(elts) for elts in lines if len(elts) == 2]
      return [input_data[i:i+self.image_batch_size] for i in range(0,len(input_data), self.image_batch_size)]


    def run_inference_on_image(self, sess, img_id, img_url, node_lookup):
    ...
    def apply_inference_on_batch(self, batch):
        ...
        with tf.Graph().as_default() as g:
            ...
            with tf.Session() as sess:
                labeled = [self.run_inference_on_image(sess, img_id, img_url, self.node_lookup_bc.value) for (img_id, img_url) in batch]
